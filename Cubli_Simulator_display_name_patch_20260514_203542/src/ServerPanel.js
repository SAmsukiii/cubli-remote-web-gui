...
function clientDisplayName(client = {}) {
  return String(client.displayName || client.clientName || '').trim();
}

function clientLabel(client = {}) {
  return clientDisplayName(client) || shortClientId(client.clientId);
}

function findClient(access = {}, clientId = '') {
  const clients = Array.isArray(access.clients) ? access.clients : [];
...
function IdentitySection({ serverSync, role, onChangeDisplayName }) {
  const displayName = serverSync.displayName || serverSync.clientName || 'Unnamed';
    ...
    <div className="serial-control-card rounded p-3 mb-3">
      <div className="d-flex justify-content-between align-items-start gap-2">
        <div className="text-break">
          ...
          <div className="server-small-note">clientId: {shortClientId(serverSync.clientId)}</div>
        </div>
...
function AdminLoginSection({ serverSync, role }) {
  ...
  const handleLogin = async () => {
    const ok = await serverSync.loginAdmin?.({ adminId, password });
    if (ok) {
      setPassword('');
    ...
    <div className="serial-control-card rounded p-3 mb-3">
      <div className="d-flex justify-content-between align-items-start gap-2 mb-2">
        <div>
          ...
          <div className="server-small-note text-break">Client ID: {serverSync.clientId}</div>
        </div>
        <Badge bg={roleVariant(role)}>{role.toUpperCase()}</Badge>
      </div>

      <div className="d-grid gap-2">
        {isAdmin ? (
          <Button variant="outline-warning" onClick={serverSync.logoutAdmin}>
            Logout Admin
          </Button>
        ) : (
      ...
      {!isAdmin && showLoginForm ? (
        <div className="mt-3">
          ...
          {serverSync.lastError ? <Alert variant="danger" className="mt-3 mb-0 py-2">{serverSync.lastError}</Alert> : null}
        </div>
      ) : null}
    </div>
  );
    ...
    <div className="serial-control-card rounded p-3 mb-3">
      ...
      <Form.Group className="mb-3">
        ...
        <Form.Control
          size="sm"
          type="text"
          value={serverSync.serverUrl}
          onChange={(event) => serverSync.setServerUrl(event.target.value)}
          placeholder="http://localhost:5050"
        />
      </Form.Group>

      <div className="d-grid gap-2">
        <Button variant="outline-info" onClick={serverSync.testConnection} disabled={serverSync.connectionStatus === 'testing'}>
          Test Connection
        </Button>
      </div>
...
function SessionSection({ serverSync }) {
  const sessionActive = serverSync.isSessionActive;

  const handleDownload = () => {
    if (!serverSync.downloadUrl) return;
    window.open(serverSync.downloadUrl, '_blank', 'noopener,noreferrer');
  };

  return (
    <>
      <div className="serial-control-card rounded p-3 mb-3">
        <div className="serial-section-title mb-2">Session</div>
        <Row className="g-2 mb-3">
          <Col xs={6}>
            <Button variant="outline-success" className="w-100" onClick={serverSync.startSession} disabled={sessionActive}>
              Start Server Session
            </Button>
          </Col>
          <Col xs={6}>
            <Button variant="outline-danger" className="w-100" onClick={serverSync.stopSession} disabled={!sessionActive}>
              Stop Session
            </Button>
          </Col>
        </Row>
        ...
        <div className="serial-value-card rounded p-2">
          <ValueRow label="Session ID" value={serverSync.sessionId || '-'} />
          <ValueRow label="Started" value={formatDateTime(serverSync.sessionStartedAt)} />
          <ValueRow label="Ended" value={formatDateTime(serverSync.sessionEndedAt)} />
        </div>
      </div>
      ...
      <div className="serial-control-card rounded p-3 mb-3">
        <div className="d-flex justify-content-between align-items-center gap-2 mb-3">
          <div>
            <div className="serial-section-title">Upload</div>
            <div className="server-small-note">Samples and command events are uploaded in batches.</div>
          </div>
          <Form.Check
            type="switch"
            id="server-auto-upload"
            label="Auto"
            checked={serverSync.autoUpload}
            onChange={(event) => serverSync.setAutoUpload(event.target.checked)}
          />
        </div>

        <Form.Group className="mb-3">
          <Form.Label className="serial-mini-label">Upload rate: {serverSync.uploadRateHz} Hz</Form.Label>
          <Form.Range
            min="0.2"
            max="5"
            step="0.2"
            value={serverSync.uploadRateHz}
            onChange={(event) => serverSync.setUploadRateHz(event.target.value)}
        ...
        <Row className="g-2 mb-3">
          <Col xs={6}>
            <Button variant="outline-light" className="w-100" onClick={serverSync.flushNow} disabled={!sessionActive || !serverSync.autoUpload || serverSync.isUploading}>
              Flush Now
            </Button>
          </Col>
          <Col xs={6}>
            <Button variant="outline-secondary" className="w-100" onClick={serverSync.clearQueues}>
              Clear Queue
            </Button>
          </Col>
        </Row>
        ...
        <div className="serial-value-card rounded p-2">
          <ValueRow label="Sample queue" value={serverSync.queueLength} />
          <ValueRow label="Event queue" value={serverSync.eventQueueLength} />
          <ValueRow label="Uploaded samples" value={serverSync.uploadedSampleCount} />
          <ValueRow label="Uploaded events" value={serverSync.uploadedEventCount} />
          <ValueRow label="Failed uploads" value={serverSync.failedUploadCount} />
          <ValueRow label="Last upload" value={formatDateTime(serverSync.lastUploadAt)} />
        </div>
      </div>

      <div className="serial-control-card rounded p-3 mb-3">
        <div className="serial-section-title mb-2">Download</div>
        <Button variant="outline-light" className="w-100" onClick={handleDownload} disabled={!serverSync.downloadUrl}>
...
function AdminManagementPanel({ serverSync, serial, status, access, role, controllerClientId, commandOwner }) {
  ...
  const connectedViewers = connectedClients.filter((client) => !client.isAdmin && client.clientId !== serverSync.clientId);
  ...
  const controllerValue = controllerClientId
    ...
    : '-';
  const myName = serverSync.displayName || access.displayName || access.clientName || shortClientId(serverSync.clientId);
  ...
  return (
    <div className="serial-control-card rounded p-3 mb-3">
      ...
      <div className="serial-value-card rounded p-2 mb-3">
        ...
        <ValueRow label="My clientId" value={shortClientId(serverSync.clientId)} />
        <ValueRow label="Current controller" value={controllerValue} />
        <ValueRow label="Command owner" value={commandOwner} />
        <ValueRow label="Connected clients" value={access.connectedClientCount ?? clients.filter((client) => client.connected).length} />
      </div>
      ...
      <Button
        ...
        onClick={serial.sendEmergencyStop}
        disabled={!status.bridge?.adminBridgeLive}
      >
        Emergency Stop
      </Button>
          ...
          <Accordion.Body>
            ...
            <div className="d-grid gap-2">
              {connectedClients.map((client) => {
                const effectiveRole = client.effectiveRole || client.role || 'viewer';
                const isMe = client.clientId === serverSync.clientId || client.isMe;
                const canGrant = !isMe && !client.isAdmin && effectiveRole !== 'controller' && client.connected;
                const canRevoke = !isMe && client.isController;
                ...
                return (
                  <div key={client.clientId} className="serial-value-card rounded p-2">
                    <div className="d-flex justify-content-between align-items-start gap-2">
                      <div className="text-break">
                      ...
                      {!client.isAdmin ? (
                        <div className="d-grid gap-1">
                          {canRevoke ? (
                            <Button variant="outline-warning" size="sm" onClick={serial.revokeControl}>
                              Revoke Control from {name}
                            </Button>
                          ) : (
                            <Button variant="outline-info" size="sm" onClick={() => serial.grantControl(client.clientId)} disabled={!canGrant}>
      ...
      <Row className="g-2 mt-3">
        <Col xs={6}>
          <Button variant="outline-warning" className="w-100" onClick={serial.revokeControl} disabled={!controllerClientId}>
            Revoke Control
          </Button>
        </Col>
        <Col xs={6}>
          <Button variant="outline-secondary" className="w-100" onClick={serial.resetAccessState}>
            Reset Server State
          </Button>
        </Col>
      </Row>

      {status.lastError ? <Alert variant="danger" className="mt-3 mb-0 py-2 text-break">{status.lastError}</Alert> : null}
    </div>
  );
}
...
function CommandSection({ serial, status, role, controllerClientId, isController }) {
  ...
  const bridge = status.bridge || serial.bridge || {};
  ...
  const lastBridgeCommand = bridge.lastBridgeCommand || status.lastBridgeCommand || serial.lastBridgeCommand || null;
  ...
  const sendShortcut = (commandKey, label, params = {}) => serial.sendEbimuShortcut?.(commandKey, label, params);
  const sendAccFactor = (value) => serial.sendAccFactor?.(Number(value) || 10);
    ...
    <div className="serial-control-card rounded p-3 mb-3">
      ...
      <Accordion defaultActiveKey="control" flush alwaysOpen className="command-accordion">
        <CommandAccordionItem eventKey="control" title="Control">
          <CommandGroup>
            <CommandButton label="Set Zero / Tare" onClick={serial.sendTare} disabled={!canSendCommand} />
            <CommandButton label="Stop" onClick={serial.sendStop} disabled={!canSendCommand} />
          </CommandGroup>
          ...
          <Row className="g-2 align-items-end">
            ...
            <Col xs={12}>
              <Button
                ...
                onClick={() => serial.sendTarget(Number(targetRoll) || 0, Number(targetPitch) || 0, Number(targetYaw) || 0)}
              >
        ...
        <CommandAccordionItem eventKey="receiver" title="Receiver Info">
          <CommandGroup>
            <CommandButton label="Status" onClick={serial.sendStatus} disabled={!canSendCommand} />
            <CommandButton label="MAC Info" onClick={serial.sendMacInfo} disabled={!canSendCommand} />
            <CommandButton label="Refresh Status" onClick={serial.refreshStatus} disabled={false} />
          </CommandGroup>
        </CommandAccordionItem>
      </Accordion>
    </div>
...
function MonitoringSection({ status }) {
  const latest = useMemo(() => status.latestPacket || {}, [status.latestPacket]);
  const lastCommandInfo = useMemo(() => status.lastCommandInfo || {}, [status.lastCommandInfo]);
  const latestDesired = useMemo(() => status.latestDesiredAttitude || {}, [status.latestDesiredAttitude]);
  ...
  const sharedRows = useMemo(() => [
    { label: 'Shared Live Data', value: status.liveStatus || (latest?.publishedAt ? 'LIVE' : 'NONE') },
    { label: 'Published source', value: latest.sourceLabel || latest.source || '-' },
    { label: 'Publisher', value: latest.publisherClientId || status.publisherClientId || (latest.source === 'server-serial' ? 'server' : '-') },
    { label: 'Last publish time', value: formatDateTime(latest.publishedAt || status.publishedAt) },
  ], [latest, status.liveStatus, status.publishedAt, status.publisherClientId]);
  ...
  const wheelGraphData = useMemo(() => {
    const rows = Array.isArray(status.chartData) ? status.chartData.slice(-120) : [];
  ...
  const livePlotData = useMemo(() => {
    const rows = Array.isArray(status.chartData) ? status.chartData.slice(-240) : [];
    return rows.map((row, index) => {
      const n = (value) => {
        const number = Number(value);
  ...
  const receiverRows = useMemo(() => [
    ...
    { label: 'Baudrate', value: status.baudRate ? `${status.baudRate} bps` : '-' },
    ...
    { label: 'Valid / Invalid', value: `${status.validCount || 0} / ${status.invalidCount || 0}` },
    { label: 'Ignored', value: status.ignoredCount || 0 },
    { label: 'Warning', value: status.warningCount || 0 },
    { label: 'Last RX', value: formatDateTime(status.lastReceivedAt) },
    { label: 'Last command', value: status.lastCommand || '-' },
  ], [latest, status]);
  ...
  const timeCommandRows = useMemo(() => [
    ...
    { label: 'Session elapsed', value: formatDuration(latest.sessionElapsedMs ?? status.sessionElapsedMs) },
    { label: 'Remote timestamp', value: formatNumber(latest.ebimu_timestamp_ms ?? latest.timestamp, 0) },
  ...
  ], [latest, lastCommandInfo, status.sessionElapsedMs]);
  ...
  return (
    <>
      ...
      <div className="serial-control-card rounded p-3 mb-3">
        ...
        {showRawMonitor ? <div className="serial-raw-line">{status.lastRawLine || '-'}</div> : null}
      </div>
    </>
  );
}
...
function WebSerialBridgeDebugSection({ serverSync, status, isAdmin, webSerialConnected, webSerialLatestPacketUpdatedAt }) {
  const latestSharedPacketAgeMs = serverSync.latestSharedPacketAgeMs ?? status.latestSharedPacketAgeMs;
  const bridgeEnabled = Boolean(serverSync.bridgeEnabled);
  const rows = [
    { label: 'Current serverUrl', value: serverSync.serverUrl || '-' },
    { label: 'Publish endpoint full URL', value: serverSync.publishEndpointUrl || '-' },
    { label: 'Last publish HTTP status', value: serverSync.lastPublishHttpStatus ?? '-' },
    { label: 'Last publish time', value: formatDateTime(serverSync.lastPublishAt) },
    { label: 'Last publish error message', value: serverSync.lastPublishError || '-' },
    { label: 'Publish count', value: serverSync.publishCount ?? 0 },
    { label: 'Failed publish count', value: serverSync.publishFailedCount ?? 0 },
  ...
  return (
    <div className="serial-control-card rounded p-3 mb-3">
      <div className="d-flex justify-content-between align-items-start gap-3 mb-2">
        ...
        {isAdmin ? (
          <Form.Check
            ...
            onChange={(event) => serverSync.setBridgeEnabled?.(event.target.checked)}
          />
        ) : null}
      </div>
      ...
      {serverSync.lastPublishError ? (
        <Alert variant={serverSync.lastPublishHttpStatus === 404 ? 'danger' : 'warning'} className="py-2 mt-3 mb-0 text-break">
          {serverSync.lastPublishError}
        </Alert>
      ) : null}
      {serverSync.publishBackoffUntil && Date.now() < serverSync.publishBackoffUntil ? (
...
export default function ServerPanel({ serverSync, webSerialConnected = false, webSerialLatestPacketUpdatedAt = null, onChangeDisplayName = null }) {
  const serial = serverSync.serverSerial || {};
  const status = serial.status || {};
  const access = status.access || {};
  const role = String(serverSync.role || access.myEffectiveRole || access.role || 'viewer').toLowerCase();
  const isAdmin = role === 'admin';
  const controllerClientId = access.controllerClientId || '';
  const isController = role === 'controller' || (controllerClientId && controllerClientId === serverSync.clientId);
  const controllerClient = findClient(access, controllerClientId);
  const controllerName = access.controllerDisplayName || access.controllerClientName || clientDisplayName(controllerClient);
  ...
  return (
    <div className="server-panel serial-panel pt-2">
      <div className="d-flex justify-content-between align-items-center mb-3 gap-2">
        ...
        <Badge bg={statusVariant(serverSync.connectionStatus)}>{serverSync.connectionStatus.toUpperCase()}</Badge>
      </div>

      <IdentitySection serverSync={serverSync} role={isController ? 'controller' : role} onChangeDisplayName={onChangeDisplayName} />
      <AdminLoginSection serverSync={serverSync} role={role} />
      <ServerConnectionSection serverSync={serverSync} />
      ...
      {isAdmin ? <SessionSection serverSync={serverSync} /> : null}

...
export default function useServerSync() {
  ...
  const requestJson = useCallback((url, options = {}) => {
    if (!displayName) {
      return Promise.reject(new Error('Enter a display name before connecting to the Cubli server.'));
    }
    return fetchJson(url, {
      ...options,
      body: addClientIdentityToJsonBody(options.body, clientId, displayName),
      headers: {
        'X-Cubli-Client-Id': clientId,
        'X-Cubli-Client-Name': displayName,
        'X-Cubli-Page': typeof window !== 'undefined' ? window.location.pathname || '/' : '/',
  ...
  const discoverServerUrl = useCallback(async (preferredUrl = serverUrlRef.current) => {
    ...
    for (const candidate of candidates) {
      const data = await probeCubliServer(candidate, clientId, displayName);
      if (!data) continue;
    ...
    return '';
  }, [applyHealthData, clientId, displayName, hasDisplayName]);
  ...
  const ensureApiServerUrl = useCallback(async (preferredUrl = serverUrlRef.current, options = {}) => {
    ...
    const verified = await probeCubliServer(current, clientId, displayName);
    if (verified) {
      apiServerVerifiedUrlRef.current = current;
      apiServerVerifiedAtRef.current = Date.now();
      return current;
    ...
    const detectedUrl = await discoverServerUrl(current);
    return detectedUrl || current;
  }, [clientId, discoverServerUrl, displayName, hasDisplayName]);
  ...
  }, [displayName, refreshQueueLengths, requestJson]);
  ...
  }, [clientId, discoverServerUrl, displayName, ensureApiServerUrl, requestJson, serverSerialStatus.latestDesiredAttitude]);
  ...
  const pollBridgeCommands = useCallback(async () => {
    const baseUrl = cleanServerUrl(serverUrlRef.current);
    try {
      const data = await requestJson(`${baseUrl}/api/bridge/commands/poll?clientId=${encodeURIComponent(clientId)}&clientName=${encodeURIComponent(displayName)}`, { method: 'GET' });
      setServerSerialStatus((prev) => ({
        ...prev,
        bridge: data.bridge || prev.bridge,
        lastBridgeCommand: data.bridge?.lastBridgeCommand || prev.lastBridgeCommand,
  ...
  }, [clientId, displayName, requestJson]);
  ...
  useEffect(() => {
    ...
    const connect = () => {
      if (closed) return;
      const url = `${baseUrl}/api/live/stream?clientId=${encodeURIComponent(clientId)}&clientName=${encodeURIComponent(displayName)}`;
  ...
  }, [clientId, displayName, hasDisplayName, scheduleLiveStateFlush, serverUrl]);

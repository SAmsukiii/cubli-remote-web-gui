...
function rememberClient(identity) {
  ...
  const previous = accessState.clients.get(identity.clientId) || {};
  const displayName = sanitizeClientName(identity.displayName || identity.clientName || previous.displayName || previous.clientName || '');
  accessState.clients.set(identity.clientId, {
    clientId: identity.clientId,
    displayName,
    clientName: displayName,
    role: getBaseRole(identity.clientId),
    connectedAt: previous.connectedAt || new Date().toISOString(),
    lastSeenAt: new Date().toISOString(),
    page: identity.page || previous.page || '',
    source: identity.source || previous.source || '',
  });
}
...
function publicAccessState(forClientId = '') {
  ...
  cleanupStaleController();
  const now = Date.now();
  const clients = Array.from(accessState.clients.values()).map((client) => {
    ...
    return {
      ...
      displayName,
      clientName: displayName,
      role: getBaseRole(client.clientId),
      effectiveRole: getEffectiveRole(client.clientId),
      isAdmin: client.clientId === accessState.adminClientId,
      isController: client.clientId === accessState.controllerClientId,
      isMe: client.clientId === forClientId,
      connected,
      stale: !connected,
  ...
  const role = getEffectiveRole(forClientId);
  const selfName = getStoredClientName(forClientId);

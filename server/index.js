...
const ENCODER_ALIGNMENT_MAX_AGE_MS = 500;
const ENCODER_ALIGNMENT_SOURCE = 'pwa-latest-encoder-cache';
...
const COMPACT_LIVE_PACKET_KEYS = Object.freeze([
  ...
  'enc_yaw_aligned_deg',
  'encoderRollAlignedDeg',
  'encoderPitchAlignedDeg',
  'encoderYawAlignedDeg',
  'dot_raw',
  'dotRaw',
  'dot_abs',
  'dotAbs',
...
function buildLivePayload(packet, options = {}) {
  ...
  const latestPacket = sharedState.latestSharedPacket
    ? attachEncoderAlignment(sharedState.latestSharedPacket, {
      now: serverSentAt,
      encoderEulerSequence: sharedState.latestSharedPacket.encoderEulerSequence,
      encoderDisplayRollSign: sharedState.latestSharedPacket.encoderDisplayRollSign,
      encoderDisplayPitchSign: sharedState.latestSharedPacket.encoderDisplayPitchSign,
      encoderDisplayYawSign: sharedState.latestSharedPacket.encoderDisplayYawSign,
    })
    : null;
  const compactPacket = compactLivePacket(latestPacket, { includeDebug });
...
function normalizeImuDisplaySigns(source = {}) {
...
function normalizeEncoderTelemetry(packet = {}, options = {}) {
  ...
  const rawEncoderUpdatedAt = firstStrictFinite([packet.encoderUpdatedAt, nested.updatedAt], null);
...
function eulerDegToQuat(rollDeg, pitchDeg, yawDeg, sequence = 'ZYX') {
...
function encoderAlignmentBaseFields(packet = {}, options = {}) {
  ...
  const now = finiteNumber(options.now, Date.now());
  const encoderUpdatedAt = firstStrictFinite([
  ...
  const satRaw = [
    firstStrictFinite([packet.sat_q0, packet.q0, packet.q?.[0]], null),
    firstStrictFinite([packet.sat_q1, packet.q1, packet.q?.[1]], null),
    firstStrictFinite([packet.sat_q2, packet.q2, packet.q?.[2]], null),
    firstStrictFinite([packet.sat_q3, packet.q3, packet.q?.[3]], null),
  ];
  const satAvailable = packet.hasTelemetryAttitude !== false
    && satRaw.every((value) => value !== null);
  const encRaw = [
    firstStrictFinite([packet.enc_q0_raw, packet.encoderQ0Raw, encoder.q0Raw, packet.enc_q0, packet.encoderQ0, encoder.q0], null),
    firstStrictFinite([packet.enc_q1_raw, packet.encoderQ1Raw, encoder.q1Raw, packet.enc_q1, packet.encoderQ1, encoder.q1], null),
    firstStrictFinite([packet.enc_q2_raw, packet.encoderQ2Raw, encoder.q2Raw, packet.enc_q2, packet.encoderQ2, encoder.q2], null),
    firstStrictFinite([packet.enc_q3_raw, packet.encoderQ3Raw, encoder.q3Raw, packet.enc_q3, packet.encoderQ3, encoder.q3], null),
  ];
  const satSeq = firstStrictFinite([options.satSeq, packet.seq, packet.rxCount, packet.packetCount], null);
  const satTimestampUs = firstStrictFinite([
    options.satTimestampUs,
    packet.timestamp_us,
...
function invalidEncoderAlignmentFields(packet = {}, options = {}) {
  ...
  return {
    ...
    enc_roll_raw_deg: firstStrictFinite([packet.enc_roll_raw_deg, packet.encoder_roll_deg, packet.encoderRollDeg, packet.encoder?.rollDeg], null),
    enc_pitch_raw_deg: firstStrictFinite([packet.enc_pitch_raw_deg, packet.encoder_pitch_deg, packet.encoderPitchDeg, packet.encoder?.pitchDeg], null),
    enc_yaw_raw_deg: firstStrictFinite([packet.enc_yaw_raw_deg, packet.encoder_yaw_deg, packet.encoderYawDeg, packet.encoder?.yawDeg], null),
...
function computeEncoderAlignmentFields(packet = {}, options = {}) {
  ...
  const sat = normalizeQuat(satRaw);
  const enc = normalizeQuat(encRaw);

  if (!satAvailable || !sat || !enc || encAgeMs === null || encAgeMs > maxAgeMs) {
    return invalidEncoderAlignmentFields(packet, options);
  }

  const dotRaw = quatDot(sat, enc);
  if (!Number.isFinite(dotRaw)) return invalidEncoderAlignmentFields(packet, options);

  const encAligned = dotRaw < 0 ? enc.map((value) => -value) : [...enc];
  const dotAbs = clamp(Math.abs(dotRaw), 0, 1);
  ...
  const encoderEulerSequence = normalizeEulerSequence(options.encoderEulerSequence || packet.encoderEulerSequence || packet.encoder?.eulerSequence);
  const encoderDisplaySigns = normalizeRpySigns({
...
function attachEncoderAlignment(packet = {}, options = {}) {
...
function normalizePublishedPacket(packet, source, identity, publishMeta = {}) {
  ...
  const normalized = {
    ...
    lastTelemetryAt: firstFinite([packet.lastTelemetryAt], sampleType !== 'ENC' ? publishedAt : null),
    rawPrefix,
    raw_prefix: rawPrefix,
  ...
  return {
    ok: true,
    packet: attachEncoderAlignment(normalized, {
      now: publishedAt,
      encoderEulerSequence,
      encoderDisplayRollSign: packet.encoderDisplayRollSign,
      encoderDisplayPitchSign: packet.encoderDisplayPitchSign,
...
function setLatestSharedPacket(packet, meta = {}) {
  ...
  const encoderTelemetry = normalizeEncoderTelemetry(packet, {
    ...
    encoderAngleToQuatSequence: packet.encoderAngleToQuatSequence,
    encoderDisplayRollSign: packet.encoderDisplayRollSign,
    encoderDisplayPitchSign: packet.encoderDisplayPitchSign,
    encoderDisplayYawSign: packet.encoderDisplayYawSign,
  });
  const packetForStorageBase = previousPacket && isEncoderOnlySample(packet)
  ...
  const packetForStorage = attachEncoderAlignment(packetForStorageBase, {
    now: publishedAt,
    encoderEulerSequence: packetForStorageBase.encoderEulerSequence,
    encoderDisplayRollSign: packetForStorageBase.encoderDisplayRollSign,
    encoderDisplayPitchSign: packetForStorageBase.encoderDisplayPitchSign,
    encoderDisplayYawSign: packetForStorageBase.encoderDisplayYawSign,
  });
  ...
  pushLimited(sharedState.chartData, {
    ...
    satQ0: sharedPacket.sat_q0 ?? sharedPacket.q0 ?? sharedPacket.q?.[0],
    satQ1: sharedPacket.sat_q1 ?? sharedPacket.q1 ?? sharedPacket.q?.[1],
    satQ2: sharedPacket.sat_q2 ?? sharedPacket.q2 ?? sharedPacket.q?.[2],
    satQ3: sharedPacket.sat_q3 ?? sharedPacket.q3 ?? sharedPacket.q?.[3],
    ...
    encQ0: sharedPacket.enc_q0 ?? sharedPacket.encoderQ0,
    encQ1: sharedPacket.enc_q1 ?? sharedPacket.encoderQ1,
    encQ2: sharedPacket.enc_q2 ?? sharedPacket.encoderQ2,
    encQ3: sharedPacket.enc_q3 ?? sharedPacket.encoderQ3,
    encQ0Raw: sharedPacket.enc_q0_raw ?? sharedPacket.encoderQ0Raw ?? sharedPacket.encoder?.q0Raw ?? sharedPacket.enc_q0 ?? sharedPacket.encoderQ0,
    encQ1Raw: sharedPacket.enc_q1_raw ?? sharedPacket.encoderQ1Raw ?? sharedPacket.encoder?.q1Raw ?? sharedPacket.enc_q1 ?? sharedPacket.encoderQ1,
    encQ2Raw: sharedPacket.enc_q2_raw ?? sharedPacket.encoderQ2Raw ?? sharedPacket.encoder?.q2Raw ?? sharedPacket.enc_q2 ?? sharedPacket.encoderQ2,
    encQ3Raw: sharedPacket.enc_q3_raw ?? sharedPacket.encoderQ3Raw ?? sharedPacket.encoder?.q3Raw ?? sharedPacket.enc_q3 ?? sharedPacket.encoderQ3,
    encoderRollAligned: sharedPacket.enc_valid ? sharedPacket.enc_roll_aligned_deg : null,
    encoderPitchAligned: sharedPacket.enc_valid ? sharedPacket.enc_pitch_aligned_deg : null,
    encoderYawAligned: sharedPacket.enc_valid ? sharedPacket.enc_yaw_aligned_deg : null,
    encQ0Aligned: sharedPacket.enc_valid ? sharedPacket.enc_q0_aligned : null,
    encQ1Aligned: sharedPacket.enc_valid ? sharedPacket.enc_q1_aligned : null,
    encQ2Aligned: sharedPacket.enc_valid ? sharedPacket.enc_q2_aligned : null,
    encQ3Aligned: sharedPacket.enc_valid ? sharedPacket.enc_q3_aligned : null,
    dotRaw: sharedPacket.dot_raw,
    dotAbs: sharedPacket.dot_abs,
    thetaErr: sharedPacket.theta_err_deg,
    encAgeMs: sharedPacket.enc_age_ms,
    encValid: sharedPacket.enc_valid,
  }, MAX_CHART_POINTS);
...
function sanitizeSharedState(clientId = '') {
  ...
  const packet = storedPacket
    ? compactLivePacket(attachEncoderAlignment({
      ...storedPacket,
      ...normalizeEncoderTelemetry(storedPacket, {
        now,
        encoderEulerSequence: storedPacket.encoderEulerSequence,
        encoderAngleToQuatSequence: storedPacket.encoderAngleToQuatSequence,
    ...
    : null;
  const ageMs = packet?.publishedAt ? now - packet.publishedAt : null;
  const publicPublisher = sanitizeActivePublisher(activePublisher, { includeDebug });
  const publisherStatus = publicPublisher.status || 'NONE';
  const liveStatus = publisherStatus === 'NONE'
    ? 'NO_ACTIVE_PUBLISHER'
